<template>
    <div>
        ...
        <div class="container">
            ...
            <input v-on:click="editPeriode()" class="btn btn-primary" value="enregistrer">
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        async created() {
            try {
                this.periode = await FormationService.getPeriodebyId(this.$route.params.idFormation,this.$route.params.idPeriode);
                this.nom = this.periode.nom_periode;
                this.surnom = this.periode.surnom_periode;
                this.duree = this.periode.duree;
                this.nombre_groupe_CM = this.periode.nombre_groupe_CM;
                this.nombre_groupe_TD = this.periode.nombre_groupe_TD;
                this.nombre_groupe_TP = this.periode.nombre_groupe_TP;
                this.nombre_groupe_Partiel = this.periode.nombre_groupe_Partiel;
            } catch (err) {
        ...
        methods: {
            async editPeriode(){
                await FormationService.editPeriode(this.nom,this.surnom,this.duree,this.nombre_groupe_CM,
                    this.nombre_groupe_TD,this.nombre_groupe_TP,this.nombre_groupe_Partiel,this.$route.params.idFormation,this.periode._id).then(res =>{
                    if (res.data.err === 1){
