<template>
    <div>
        ...
        <div class="container">
            ...
            <input v-on:click="createStatut()" class="btn btn-primary" value="enregistrer">
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            async createStatut(){
                await EnseignantService.addStatut(this.nom,this.surnom,this.heure_normal_min,this.heure_normal_max,
                    this.heure_supp_min,this.heure_supp_max).then(res =>{
                    if (res.data.err === 1){
